...
import time
from des_stacks.utils.stack_tools import make_good_frame_list, make_good_frame_list_chip make_swarp_cmds, get_dessn_obs, get_des_obs_year,resample
    ...
    def do_my_stack(self, cuts={'teff':0.2, 'zp':None,'psf':None},final=True):
        ...
        for chip in self.chips:
            if not os.path.isfile(os.path.join(self.list_dir,'good_exps_%s_%s_%s_%s.csv'%(field,band,chip,self.cutstring))):
                ...
                self.good_frames[chip] = make_good_frame_list_chip(self,field,chip,band,cuts)

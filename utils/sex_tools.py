...
def sex_for_psfex(stack,chip):
    ...
    try:
        ...
        stack.logger.info("Took %s seconds" %start-end)
...
def psfex(stack,chip,retval='FWHM'):
    ...
    stack.logger.info("Took %s secconds" %sstart-end)
    if retval = 'FWHM':
...
def sex_for_cat(stack,chip):
    ...
    os.chdir(ana_dir)
    sexcat = os.path.join(ana_dir,'%s_%s_%s_%s_%s.sexcat' %(stack.my,
    ...
    try:
        stack.logger.info("Running SExtractor on {0}".format(img))
        sex_process = subprocess.Popen(sex_cmd)
        sex_process.wait()
        stack.logger.info("Successfully ran SExtractor on {0}".format(img))
        end = time.time()
        stack.logger.info("Took %s seconds" %s start-end)
        stack.logger.info("Saved at {0}".format(sexcat))
        return sexcat
    except (OSError, IOError):
        stack.logger.info("SExtractor failed...")
